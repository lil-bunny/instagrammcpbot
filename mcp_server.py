...
def init_client() -> Client:
    """
    Initialize Instagram client with session reuse.
    ...
    Returns:
        Client: Configured instagrapi client
    """
    cl = Client()
    try:
        if os.path.exists(sessionfile):
            logger.info(f"Loading session from {sessionfile}")
            cl.load_settings(sessionfile)
            try:
                # Verify session validity without full login
                cl.get_timeline_feed()
                logger.info("Session loaded and valid")
            except Exception:
                logger.info("Session invalid, attempting relogin")
                cl.login(username=USERNAME, password=PASSWORD, relogin=True)
                cl.dump_settings(sessionfile)
                logger.info(f"Session refreshed and saved to {sessionfile}")
        else:
            logger.info("No session file found, performing full login")
...
@mcp.tool()
def get_post_info_from_url( url: str) -> dict:
    """
    Fetch full metadata of an Instagram post from its URL using Instagrapi.
    ...
    Returns:
        A dict containing media metadata or error.
    """
    try:
        # Extract shortcode from the Instagram URL
        match = re.search(r"(?:https?://)?(?:www\.)?instagram\.com/p/([a-zA-Z0-9_-]+)/?", url)
        if not match:
        ...
        return {
            "success": True,
            "message": "Media info fetched successfully.",
            "data": media.dict()
        }
...
@mcp.tool()
def get_top_liked_comments_till_time(url: str, timestamp: str, top_n: int = 3, max_pages: int = 10) -> dict:
    """
    Fetch the top N most liked Instagram comments before a given timestamp.

    Args:
        url (str): Instagram post URL.
        timestamp (str): ISO format timestamp (e.g., '2025-06-27T14:00:00').
        top_n (int): Number of top liked comments to return.
        max_pages (int): Number of pages (~50 comments each) to scan.

    Returns:
        dict: Contains list of top N liked comments details or failure message.
    """
    try:
        ...
        while page < max_pages:
            comments_chunk, min_id = cl.media_comments_chunk(media_id, max_amount=50, min_id=min_id)

            valid_comments = [c for c in comments_chunk if c.created_at_utc < target_time]
            all_comments.extend(valid_comments)

            if not min_id or not comments_chunk:
                break
        ...
        if not all_comments:
            return {"success": False, "message": "No comments found before the given timestamp."}

        # Sort by like count
        sorted_comments :List[Comment]= sorted(all_comments, key=lambda c: c.like_count, reverse=True)
        top_comments:List[Comment] = sorted_comments[:top_n]
...
@mcp.tool()
def unlike_comment(comment_id: int) -> dict:
    """
    Unlike an Instagram comment by its ID.

    Args:
        comment_id: PK of the comment to unlike.

    Returns:
        dict: success, message string (containing liked comment id), optionally error.
    """
    try:
        cl = init_client()
...
def like_comment(comment_id: int) -> dict:
    """
    Like an Instagram comment by its ID.
...
def reply_to_comment(media_id: str, comment_id: int, text: str,reply_username:str="") -> dict:
    """
    Reply to a specific Instagram comment using media_id and comment_id ,and reply username[optional]
    ...
    Returns:
        A dictionary with success status, message, reply comment data or error.
    """
  
    cl = init_client()
    reply = cl.media_comment(
        str(media_id),
        text = text + (f" @{reply_username}" if reply_username else ""),
...
def send_message(username: str, message: str) -> Dict[str, Any]:
    ...
    try:
        myclient=init_client()
        user_id = myclient.user_id_from_username(username)
        if not user_id:
            return {"success": False, "message": f"User '{username}' not found."}
        
        dm = myclient.direct_send(message, [user_id])
        if dm:
